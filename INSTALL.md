# tl;dr - v1.0.0 of ezpublish/ezplatform

* run `dock-cli build` or `docker-compose build`
* if needed run `dock-cli reset` 

* Once you have the containers running, run this command ` php app/console ezplatform:install --env prod clean` to fill the
database with the tables required.

go to `http://dockerezpublish_web.docker/` for the homepage, there should be no styling.
go to `http://dockerezpublish_web.docker/ez` for admin, this takes a while to load first time
`admin/publish` is the default username and password, this should be disabled for any production deployment as stated on the course.
